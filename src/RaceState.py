class RaceState:

    def __init__(self, current_lap, total_laps, tyre_compound, tyre_wear, fuel_load, lap_time_history, pit_stops, stint_num, dnf, stint_length, track, safety_car, allowed_pit_strategy):
        self.current_lap = current_lap
        self.total_laps = total_laps
        self.tyre_compound = tyre_compound
        self.tyre_wear = tyre_wear
        self.fuel_load = fuel_load
        self.lap_time_history = lap_time_history
        self.pit_stops = pit_stops
        self.stint_num = stint_num
        self.dnf = dnf
        self.stint_length = stint_length
        self.track = track
        self.safety_car = safety_car
        self.allowed_pit_strategy = allowed_pit_strategy





